    ...
    protected void drainChannel (final SelectionKey key, ObjectReader reader) throws Exception {
        ...
        for (ChannelMessage msg : msgs) {
            ...
            try {
                 ...
                 */
                if (ChannelData.sendAckSync(msg.getOptions())) sendAck(key,(WritableByteChannel)channel,Constants.ACK_COMMAND,saddr);
            }catch ( RemoteProcessException e ) {
                if ( log.isDebugEnabled() ) log.error("Processing of cluster message failed.",e);
                if (ChannelData.sendAckSync(msg.getOptions())) sendAck(key,(WritableByteChannel)channel,Constants.FAIL_ACK_COMMAND,saddr);
            }catch ( Exception e ) {
